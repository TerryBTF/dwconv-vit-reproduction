"""Train learned baselines on the controlled local-defect dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", type=Path, default=root / "generated" / "local_defects.npz")
...
def train_model(
    ...
    metrics = {split: accuracy(model, loader, device) for split, loader in loaders.items()}
    metrics["generalization_gap"] = metrics["train"] - metrics["test"]
    return metrics, duration


def summarize(rows: list[dict[str, float | int | str]]) -> list[dict[str, float | str]]:
    summaries: list[dict[str, float | str]] = []
    for model_name in ("mlp", "small_cnn"):
        selected = [row for row in rows if row["model"] == model_name]
        for metric in ("train_accuracy", "test_accuracy", "generalization_gap", "training_seconds"):
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        for model_name, model_factory in (("mlp", MLP), ("small_cnn", SmallCNN)):
            ...
            metrics, duration = train_model(
                model_factory(), loaders, device, args.epochs, args.learning_rate
            )
            row = {
                "model": model_name,
                "seed": seed,
            ...
            print(
                ...
                f"test={row['test_accuracy']:.2f}%, "
                f"gap={row['generalization_gap']:.2f} pp"
            )

    with (args.output / "learned_model_runs.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    with (args.output / "learned_model_summary.json").open("w", encoding="utf-8") as handle:
